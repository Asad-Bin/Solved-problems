...
void divs(ll n) {
	...
	for(i=0; i<(int)primes.size() && (ll)(primes[i]*primes[i])<=n; i++) {
		if(n%primes[i]==0) {
			cnt=1;
...
vl ara;
void make(int at, ll cnt)
{
	if(at==sz(divisors)) return;
	
	make(at+1, cnt);
	ll num = 1;
	for(int K=1; K<=divisors[at].se; K++){
		num *= divisors[at].fi;
		ara.PB(num*cnt);
		make(at+1, num*cnt);
	}
...
void task()
{
	...
	while(t--){
		...
		if(cnt-n<n) cout << "deficient\n";
		else if (cnt-n==n) cout << "perfect\n";
		else cout << "abundant\n";
	}
}
