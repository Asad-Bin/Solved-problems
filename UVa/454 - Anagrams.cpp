...
int main()
{
    ...
    while(t--){
		...
		while(true){
			...
			a.erase(remove_if(a.begin(), a.end(), isIgnoredChar), a.end());
			sort(all(a));
		...
		for(int K=1; K<i; K++){
			if(sz(ara[K])>1){
				for(int L=0; L<sz(ara[K])-1; L++){
					for(int M=L+1; M<sz(ara[K]); M++){
						if(ara[K][L]<=ara[K][M]) v.PB({ara[K][L], ara[K][M]});
						else v.PB({ara[K][M], ara[K][L]});
						//ss.insert({min(ara[K][L], ara[K][M]), max(ara[K][L], ara[K][M])});
					}
				}
			}
		}
