...
int main()
{
    ...
    for(int cs=1; cs<=t; cs++){
		...
		for(int K=0; K<n; K++){
			for(int L=0; L<m; L++){
				si(dp[K][L]);
				dp[K][L] += (K>0?dp[K-1][L]:0) + (L>0?dp[K][L-1]:0) - (K>0 && L>0?dp[K-1][L-1]:0);
			}
		}
		...
		for(int K=0; K<n; K++){
			for(int L=0; L<m; L++){
				for(int i=K; i<n; i++){
					for(int j=L; j<m; j++){
						cost = dp[i][j] - (L>0?dp[i][L-1]:0) - (K>0?dp[K-1][j]:0) + (K>0 && L>0?dp[K-1][L-1]:0);
						
						if(cost<=budget && (i-K+1)*(j-L+1)>ans.fi){
							ans.fi = (i-K+1)*(j-L+1);
							ans.se = cost;
						}
						else if(cost<=budget && (i-K+1)*(j-L+1)==ans.fi){
							ans.se = min(cost, ans.se);
						}
					}
				}
			}
